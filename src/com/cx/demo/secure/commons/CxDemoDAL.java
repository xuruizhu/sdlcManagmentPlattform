...
public class CxDemoDAL {
	...
	private int 		resSize		= CxDemoPost.MAX_POSTS_PER_PAGE;

	private CxDemoDAL(CxDemoProperties props) {
		try {
			this.props = props;
			this.user = props.getProperty(CxDemoProperties.KEY_DB_USER);
			this.password = props.getProperty(CxDemoProperties.KEY_DB_PASS);
			createDB();
		}
	...
	private Connection getDBConnection(String user, String password) throws CxDemoException {
		try {
			...
			return DriverManager.getConnection("jdbc:hsqldb:file:cx-demo-secure", user, password);
		} catch (Exception ex) {
	...
	public void resetUsers() throws CxDemoException {
		try {
			...
			execute("set table USERS source \"webapps\\demo-secure\\dat\\users.csv;fs=,;ignore_first=true;encoding=Shift_JIS\"", null);
	...
	public void resetPosts() throws CxDemoException {
		try {
			...
			execute("set table POSTS source \"webapps\\demo-secure\\dat\\posts.csv;fs=,;ignore_first=true;encoding=Shift_JIS\"", null);
		...
		try {
			...
			if (args != null) {
				for (int idx=0; idx<args.length; idx++) {
					statement.setString(idx+1, args[idx]);
				}
			}
